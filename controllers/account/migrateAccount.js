...
const migrateAccount =  async (req, res) => {
    ...
    try {
        ...
        const [authorRows] = await dbPromise.query("SELECT * FROM authors_account WHERE email = ?", [authorEmail]);
        ...
        const [editorRows] = await dbPromise.query("SELECT * FROM editors WHERE email = ?", [email]);
