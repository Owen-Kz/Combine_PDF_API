...
const returnPaperPage = async (req,res) =>{
    try{
        if(req.cookies.asfirj_userRegistered){
            ...
            const EmailTemplate = await getEmailTemplates(req,res, "return_manuscript")
          ...
          CorrespondingAuthorsName  = `${getAuthor[0][0].prefix} ${getAuthor[0][0].firstname} ${getAuthor[0][0].lastname} ${getAuthor[0][0].othername}`
            ...
            res.render("returnPaper", {user:req.user, articleData: getArticleData.article, messageBody:JSON.stringify(EmailTemplate.emailContent), CorrespondingAuthorsName })
        }else{
