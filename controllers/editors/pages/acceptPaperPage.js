...
const acceptPaperPage = async (req,res) =>{
    try{
        if(req.cookies.asfirj_userRegistered){
            ...
            const EmailTemplate = await getEmailTemplates(req,res, "accept_paper")
            ...
            res.render("acceptPaper", {user:req.user, articleData: getArticleData.article, messageBody:JSON.stringify(EmailTemplate.emailContent),CorrespondingAuthorsName  })
        }else{
